...
import bookAudio from "./assets/book.mp3";

const targetDate = new Date("2026-09-20T10:00:00");
const GOOGLE_APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwxiFb9qupIyRPMQ5NHvSxJUC66Fmg3TlE3HUNtD5EcsnK7dKNSlNcZG9l36C035Bew/exec";

const pageBackgroundStyle = {
  backgroundImage: `linear-gradient(180deg, rgba(223, 249, 255, 0.82) 0%, rgba(200, 241, 255, 0.75) 38%, rgba(245, 251, 255, 0.88) 100%), url(${backgroundImage})`,
...
function App() {
  ...
  useEffect(() => {
    ...
    audioRef.current = new Audio(bookAudio);
  ...
  const handleOpen = () => {
    ...
    if (audioRef.current) {
      ...
      audioRef.current.play().catch(() => {
        // Browser may block autoplay until user interaction; this click is allowed.
      });
    }
  ...
  const handleAttendanceChange = (nextChoice) => {
    setRsvpChoice(nextChoice);
  ...
  const handleRsvpSubmit = async (event) => {
    ...
    const isAttending = rsvpChoice === "yes";
    const trimmedName = guestName.trim();
    const finalAdultCount = isAttending ? Number(adultCount) || 0 : 0;
    const finalChildCount = isAttending ? Number(childCount) || 0 : 0;
    const payload = {
      attending: isAttending,
      guestName: trimmedName || "Guest",
      adults: finalAdultCount,
      children: finalChildCount,
      totalGuests: finalAdultCount + finalChildCount,
    ...
    try {
      const scriptUrl = GOOGLE_APPS_SCRIPT_URL.trim();

      if (scriptUrl && scriptUrl !== "PASTE_YOUR_DEPLOYMENT_URL_HERE") {
        ...
        console.log("Google Apps Script URL not configured yet. Running in local preview mode.", payload);
      }
    } catch (error) {
      console.error("RSVP submission failed:", error);
      setSubmitError("Your RSVP could not be sent right now. Please try again.");
            ...
            <form className="mt-5 space-y-4" onSubmit={handleRsvpSubmit}>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  ...
                  className="rounded-full bg-[#60cdee] px-5 py-2.5 font-semibold uppercase text-white shadow-lg shadow-violet-300/40 transition hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-70"
  ...
  if (showDetails) {
    return (
      <>
        ...
        >
          <div className="reveal-in">
            <div className="mb-7 flex justify-center text-center">
              <div className="inline-flex items-center rounded-full border border-[#60cdee]/40 bg-white/70 px-6 py-3 shadow-[0_12px_30px_rgba(96,205,238,0.18)] backdrop-blur-sm">
                <p className="font-serif text-2xl tracking-[0.18em] text-slate-700 md:text-3xl">You&apos;re Invited</p>
              </div>
            </div>
        ...
        >
          ...
          <section className="relative flex flex-col items-center justify-center overflow-hidden rounded-3xl p-6 md:p-10">
            ...
            <div className="mt-3 flex flex-col items-center text-center text-[#60cdee]">
              <p>Your presence is a gift, but if you want to spoil me please check our registry.</p>
            </div>
            <a
        ...
        <section className="mx-auto mt-5 max-w-4xl rounded-[2rem] border border-[#60cdee]/25 bg-gradient-to-r from-white via-sky-50 to-cyan-50 p-6 shadow-[0_18px_40px_rgba(96,205,238,0.18)] backdrop-blur-sm">
          <div className="flex flex-col items-center justify-center gap-4 text-center">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.32em] text-[#60cdee]">RSVP</p>
              <h3 className="mt-2 font-serif text-3xl text-slate-800">We would love to celebrate with you.</h3>
            </div>

            <button
              type="button"
              onClick={() => setIsRsvpModalOpen(true)}
              className="rounded-full bg-[#60cdee] px-7 py-3.5 font-semibold uppercase text-white shadow-lg shadow-cyan-300/40 transition hover:scale-[1.02]"
              style={{ letterSpacing: "0.14em" }}
            >
              RSVP
            </button>
          ...
          {rsvpConfirmation ? (
            <div className="mt-5 flex justify-center">
              <p className="text-center text-sm font-medium text-slate-700">
                Thank you, {rsvpConfirmation}! Your RSVP has been recorded.
              </p>
            </div>
          ) : null}
        </section>
